$(document).ready(function () {
  // 문서가 로드된 후에 JavaScript 코드가 적절한 시점에 실행하게 해주는 코드
  ...
  $(".prev_btn").click(function (e) {
    // 이전 버튼(.prev_btn)을 클릭했을 때 실행될 함수를 등록. back() 함수를 호출하여 이전 이미지로 이동
    back();
  });

  $(".next_btn").click(function (e) {
    // 다음 버튼(.next_btn)을 클릭했을 때 실행될 함수를 등록. next() 함수를 호출하여 다음 이미지로 이동
  ...
  function back() {
    // back()는 이전 이미지로 이동하는 동작을 수행. 현재 이미지 위치(img_position)가 1보다 큰 경우
    if (1 < img_position) {
      imgs.animate({
  ...
  function next() {
    // next()는 다음 이미지로 이동하는 동작을 수행. 현재 이미지 위치(img_position)가 이미지 개수(img_count)보다 작은 경우
